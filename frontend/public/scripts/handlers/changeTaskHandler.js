...
taskContainer.addEventListener('click', async (e) => {
    ...
    if (target.closest('div').parentElement.className == 'task' || target.closest('div'.className == 'task')) {
        
        let taskEl = target.closest('div').parentElement.className == 'task' ? target.closest('div').parentElement : target.closest('div');
